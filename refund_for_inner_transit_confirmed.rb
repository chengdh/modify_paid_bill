...
from_branchs.each do |f|
  ...
  to_branchs.each do |t|
    ...
    :state_in => ['transit_refunded_confirmed'],
    ...
    if bills.present?
      ...
      refund = InnerTransitRefund.new(:state => 'transit_refunded_confirmed',:from_org_id => t.id,:to_org_id => f.id,:bill_date => refund_date,:user_id => admin.id,:note => "总部已确认-系统自动生成返款清单")
